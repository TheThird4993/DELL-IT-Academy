...
int* batalha(Empresa* emp, int num_empresas) {
	int** matriz_batalhas;
	int sel = 0, cont = 1;
	static int vet_cont = 0, teste = 0;
	int um_vencedor = 0, num_batalhas = 0;
	int perdedor[TAM] = { 0 };
	Empresa* empresas;
	...
	while (1) {
		...
		while (!um_vencedor) {
			...
			while (1) {
				...
				while (matriz_batalhas[0][sel - 1] == -1) sel++;

				printf("\nvalor: %d\n\n", matriz_batalhas[0][sel - 1]);

				if (sel <= 0 || sel > 4) printf("Selecao invalida\n");
				else break;
			}

			for (int i = 0; i < num_empresas; i++) empresas[i].eventos = 31; /// 0001 1111 -> flags para eventos

			perdedor[vet_cont++] = rodada(matriz_batalhas[0][sel - 1], matriz_batalhas[1][sel - 1], empresas);

			for (int i = 0; i < 2; i++) matriz_batalhas[i][sel - 1] = -1;
